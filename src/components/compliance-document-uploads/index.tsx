...
const ComplianceDocumentUploader: React.FC<ComplianceDocumentUploaderType> = (
  ...
  return (
    <div className="flex justify-between  py-5 border-t border-[#2022240D]">
      <p className="text-sm text-[#7D8592]">{props.title}</p>
      <div className="flex items-center gap-x-4">
        {props.file && (
          <button className="flex items-center gap-x-2">
            <span className="text-sm text-[#20222466] underline">
              {props.file.name}
            </span>
            <span>
              <svg
        ...
        >
          {!props.file && (
            <span className="text-sm underline text-[#15707A]">Upload</span>
          )}
          <span>
            <svg
              width="21"
